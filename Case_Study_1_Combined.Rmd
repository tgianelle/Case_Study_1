...
df_bev <- data.frame(merge_files$ABV, merge_files$IBU)
col_headings <- c("ABV", "IBU")
names(df_bev) <- col_headings
res <- cor(df_bev, use="complete.obs")
round(res,2)

##ABV <- data.frame(beers$ABV, na.rm = TRUE)
##IBU <- data.frame(beers$IBU, na.rm = TRUE)
df_bev[complete.cases(df_bev),]
res2 <- rcorr(as.matrix(df_bev))
res2
...
ggsp <- ggplot(data=df_bev, aes(x=ABV, y=IBU)) +
